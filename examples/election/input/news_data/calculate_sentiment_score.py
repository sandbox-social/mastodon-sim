...
def main(headlines):
    ...
    return {
        "bill": {
            "avg_log_ratio": avg_bill_log_ratio,
            "se_log_ratio": se_bill_log_ratio
        },
        "bradley": {
            "avg_log_ratio": avg_bradley_log_ratio,
            "se_log_ratio": se_bradley_log_ratio
        }
    }
...
def plot_bias_scores(neutral_sentiments, bill_bias_sentiments, bradley_bias_sentiments):
    ...
    rects2 = ax.bar(x + width/2, bradley_ratios, width, label='Bias for\nBradley', color='#2E86C1', yerr=bradley_se, capsize=3)
